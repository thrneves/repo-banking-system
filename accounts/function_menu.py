...
def function_menu(account: dict) -> dict:
    ...
    while True:
        ...
        match user_input.lower():
            ...
            case "r":
                filter_choice = input(
                    "Filtrar por (d) depósitos, (s) saques ou Enter p/ todas: "
                ).lower()
                transaction_type = {"d": "deposit", "s": "withdrawal"}.get(
                    filter_choice
                )
                ...
                for transaction in report_generator(account, transaction_type):
                    if transaction["type"] in dict_type:
                        print(f'{dict_type[transaction["type"]]}: R$\
                            {transaction["value"]:.2f}  Em: \
                            {transaction["date"]}')
                print("=========================================")
